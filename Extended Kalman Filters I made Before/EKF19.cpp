...
void AdvancedEKF::detectStaticState(const Vector3& gyro, double dt) {
    ...
    double accel_var_threshold = 0.01;  // Reduced threshold
    
    // Current motion indicators
    bool gyro_static = (gyro_mag < 0.005);  // Tighter threshold: 0.005 rad/s
    bool accel_stable = (accel_variance_sum < accel_var_threshold);
    
    bool currently_static = gyro_static && accel_stable;
    ...
    // State transition with longer confirmation time
    if (!is_static && static_time > 0.5) {  // Require 0.5s of static
        is_static = true;
        std::cout << "Entering static mode at t=" << frame_count * dt << "s" << std::endl;
    } else if (is_static && static_time < 0.1) {
        is_static = false;
        std::cout << "Exiting static mode at t=" << frame_count * dt << "s" << std::endl;
    }
}
...
void AdvancedEKF::update(const SensorData& data, float dt) {
    ...
    // CRITICAL FIX 1: Remove the accelerometer calibration that causes roll offset
    // DELETE any lines that modify accel based on frame_count == 1
    ...
    // CRITICAL FIX 2: Proper quaternion integration with consistent canonical form
    if (corrected_gyro.norm() > 1e-8) {
        // Use exponential map for accuracy
        ...
        if (angle > 1e-8) {
            ...
            // CRITICAL: Always enforce canonical form
            if (quat_nominal_.w < 0) {
                quat_nominal_ = -quat_nominal_;
            }
        }
    ...
    // CRITICAL FIX 3: Proper NED velocity update
    Eigen::Matrix3f R_body_to_NED = quat_nominal_.toMatrix3f();
    ...
    // CRITICAL FIX 4: Correct NED gravity (positive Z direction)
    vel_NED_.z += GRAVITY_MSS * dt;  // +9.81, not -9.81
    ...
    // CRITICAL FIX 5: More conservative accelerometer fusion
    float accel_mag = accel_filtered.norm();
    if (accel_mag > 9.0f && accel_mag < 10.5f) {  // Stricter range around 1g
        correctWithAccelerometer(accel_filtered);
    }
    
    // CRITICAL FIX 6: Enhanced yaw stabilization
    if (is_static && static_time > 2.0f) {
        stabilizeYaw(dt);
    }
    
    // Update bias only during confirmed static periods
    if (is_static && static_time > 1.0f) {
        updateGyroBias(gyro, dt);
    }
