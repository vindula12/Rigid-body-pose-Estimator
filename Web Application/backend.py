...
class EKF21Wrapper:
    ...
    def __init__(self, executable_path="./EKF21"):
        ...
        self.output_file = os.path.join(self.temp_dir, "output2.csv")  # Exact filename from your command
    ...
    async def run_ekf21(self, progress_callback=None):
        ...
        try:
            ...
            cmd = ["./EKF21", "Input.csv", "output2.csv"]
    ...
    def load_results(self):
        """Load results from EKF21 output file"""
        try:
            ...
            # Expected columns from EKF21
            expected_columns = ['Timestamp', 'Roll', 'Pitch', 'Yaw', 'Quat_w', 'Quat_x', 'Quat_y', 'Quat_z']
            
            # Check for missing columns
            missing_cols = [col for col in expected_columns if col not in df.columns]
            if missing_cols:
                logger.warning(f"Missing columns in EKF21 output: {missing_cols}")
                ...
                missing_cols = [col for col in expected_columns if col not in df.columns]
                if missing_cols:
                    raise ValueError(f"Missing columns in EKF21 output: {missing_cols}")
            
            # Convert to list of dictionaries for JSON serialization
            results = []
            for i, row in df.iterrows():
                try:
                    # Simple position integration (dead reckoning)
                    # In a real implementation, this would come from EKF21 if it outputs position
                    pos_x = 0.0  # These would come from EKF21 if it outputs position
                    pos_y = 0.0
                    pos_z = 0.0
                    ...
                    result = {
                        ...
                        'pos_y': pos_y,
                        'pos_z': pos_z,
                        'vel_x': 0.0,  # Will be calculated later
                        'vel_y': 0.0,
                        'vel_z': 0.0
                    }
            ...
            for i in range(1, len(results)):
                ...
                if dt > 0:
                    results[i]['vel_x'] = (results[i]['pos_x'] - results[i-1]['pos_x']) / dt
                    results[i]['vel_y'] = (results[i]['pos_y'] - results[i-1]['pos_y']) / dt
                    results[i]['vel_z'] = (results[i]['pos_z'] - results[i-1]['pos_z']) / dt
