...
public:
    ...
    struct BiasEstimator {
        ...
        void updateGyroStats(const Vector3& gyro, float dt) {
            ...
            samples++;
            float alpha = 1.0f / samples;
            
            Vector3 delta = gyro - gyro_mean;
            gyro_mean += delta * alpha;
...
public:
    ...
    void updateWithDelayedFusion(const SensorData& data, float dt) {
        ...
        if (gyro.norm() < 0.01f) {  // Static threshold
            ...
            if (static_time > 1.0f) {  // After 1 second static
                delta_angle_bias_ = delta_angle_bias_ * 0.99f + gyro * 0.01f;
            }
        }
    ...
    void update(const SensorData& data, float dt) {
        ...
        // Fuse accelerometer
        if (fabs(accel.norm() - 9.81f) < 2.0f) {
            // For now, use simple fusion until delayed fusion is working
            Vector3 gravity_ned(0, 0, 9.81f);
            Eigen::Matrix3f R_ned_to_body = quat_nominal_.toMatrix3f().transpose();
            Vector3 expected_gravity = Vector3(R_ned_to_body * gravity_ned.toEigen());
            
            Vector3 error = expected_gravity.cross(accel.normalized());
            
            // Check innovation
            float error_mag = error.norm();
            if (error_mag < 0.5f) {  // Simple threshold
                float gain = 0.01f;
                Quaternion correction = Quaternion::fromAxisAngle(error, gain);
                ...
                // This is an outlier
                _innov_monitor.accel_healthy = false;
                _innov_monitor.accel_test_ratio = error_mag / 0.5f;
            }
        }
    ...
    void detectStaticState(const Vector3& gyro, double dt) {
        ...
        currently_static = (mean < 0.02) && (variance < 0.0001) && (range < 0.03);
