...
public:
    ...
    struct BiasEstimator {
        ...
        Vector3 gyro_accumulator;
        int sample_count = 0;
        
        void updateGyroStats(const Vector3& gyro, float dt) {
            sample_count++;
            gyro_accumulator += gyro;
            
            // Calculate bias as simple average
            gyro_bias = gyro_accumulator / (double)sample_count;
...
void AdvancedEKF::update(const SensorData& data, float dt) {
    ...
    // ALWAYS populate IMU buffer (for Test 1)
    ImuSample sample;
    ...
    // ALWAYS update bias estimator (for Test 2)
    _bias_estimator.updateGyroStats(gyro, dt);
    
    // ALWAYS test innovation monitor (for Test 3)
    ...
    if (!initialized) {
        ...
        if (accel_buffer.size() >= 10) {
            ...
            P = Eigen::Matrix<float, 23, 23>::Identity() * 0.1f;
            error_state_.setZero();
            initialized = true;
        }
        return;
