...
public:
    ...
    Vector3 toEulerAngles() const {
        ...
        double r11 = 2*(qw*qw + qx*qx) - 1;
        double r12 = 2*(qx*qy - qw*qz);
        double r13 = 2*(qx*qz + qw*qy);
        double r21 = 2*(qx*qy + qw*qz);
        double r22 = 2*(qw*qw + qy*qy) - 1;
        double r23 = 2*(qy*qz - qw*qx);
        double r31 = 2*(qx*qz - qw*qy);
        double r32 = 2*(qy*qz + qw*qx);
        double r33 = 2*(qw*qw + qz*qz) - 1;
        
        // Extract Euler angles from rotation matrix (ZYX convention)
        // Check for gimbal lock
        double sin_pitch = -r31;
        if (std::abs(sin_pitch) >= 1.0) {
            pitch = std::copysign(PI / 2.0, sin_pitch); // ±90 degrees (in radians)
            
            // When pitch = ±90°, we have gimbal lock
            if (sin_pitch < 0) {  // pitch = +90°
                roll = std::atan2(-r12, r22);
                yaw = 0;  // Set yaw to 0 (arbitrary)
            } else { // pitch = -90°
                roll = std::atan2(r12, r22);
                yaw = 0;  // Set yaw to 0 (arbitrary)
            }
        ...
        // Return angles in RADIANS (no conversion needed)
        // Normalize angles to [-π, π] range
        while (roll > PI) roll -= 2*PI;
        while (roll < -PI) roll += 2*PI;
        while (pitch > PI) pitch -= 2*PI;
        while (pitch < -PI) pitch += 2*PI;
        while (yaw > PI) yaw -= 2*PI;
        while (yaw < -PI) yaw += 2*PI;
        
        return Vector3(roll, pitch, yaw);  // Returns radians
    }
...
public:
    ...
    void josephFormCovarianceUpdate(
        ...
        if (!isPositiveDefinite(P)) {
            ...
            P += 1e-6f * Eigen::Matrix<float, DIM_X, DIM_X>::Identity();
        }
    }
    ...
    // REPLACE WITH COMPREHENSIVE PROCESS NOISE:
    void updateProcessNoise(float dt) {
        Q.setZero();
        
        // Attitude prediction noise (gyro noise)
        Q.block<3,3>(0, 0) = Eigen::Matrix3f::Identity() * (0.001f * dt);
        
        // Velocity prediction noise (accel noise)  
        Q.block<3,3>(3, 3) = Eigen::Matrix3f::Identity() * (0.1f * dt);
        
        // Position prediction noise (velocity uncertainty)
        Q.block<3,3>(6, 6) = Eigen::Matrix3f::Identity() * (0.01f * dt * dt);
        
        // Gyro bias random walk
        Q.block<3,3>(9, 9) = Eigen::Matrix3f::Identity() * (0.0001f * dt);
        
        // Accelerometer bias random walk
        Q.block<3,3>(12, 12) = Eigen::Matrix3f::Identity() * (0.001f * dt);
        
        // Magnetic field states (very slow variation)
        Q.block<3,3>(15, 15) = Eigen::Matrix3f::Identity() * (1e-6f * dt);
        Q.block<3,3>(18, 18) = Eigen::Matrix3f::Identity() * (1e-6f * dt);
        
        // Wind states  
        Q.block<2,2>(21, 21) = Eigen::Matrix2f::Identity() * (0.01f * dt);
    }
    ...
    void propagateCovariance(float dt) {
        ...
        Eigen::Matrix<float, 23, 23> P_temp = F * P * F.transpose() + Q;
    ...
    //     // Allan variance parameters (tune these for your IMU)
    //     float gyro_noise_density = 0.003f;      // rad/s/√Hz
    //     float gyro_bias_instability = 0.0001f;  // rad/s
    //     float accel_noise_density = 0.05f;      // m/s²/√Hz
    //     float accel_bias_instability = 0.001f;  // m/s²
        
    //     void updateGyroStats(const Vector3& gyro, float dt) {
    //         static Vector3 gyro_mean(0, 0, 0);
    //         static Vector3 gyro_sum(0, 0, 0);
    //         static int samples = 0;
            
    //         samples++;
    //         gyro_sum += gyro;
    //         gyro_mean = gyro_sum / samples;
            
    //         // Update bias estimate
    //         if (samples > 10) {
    //             float alpha = 0.01f;
    //             gyro_bias = gyro_bias * (1.0f - alpha) + gyro_mean * alpha;
                
    //             // Update variance
    //             Vector3 delta = gyro - gyro_mean;
    //             gyro_bias_var[0] = (1.0f - alpha) * gyro_bias_var[0] + alpha * delta.x * delta.x;
    //             gyro_bias_var[1] = (1.0f - alpha) * gyro_bias_var[1] + alpha * delta.y * delta.y;
    //             gyro_bias_var[2] = (1.0f - alpha) * gyro_bias_var[2] + alpha * delta.z * delta.z;
    //         }
    ...
    struct BiasEstimator {
        ...
        void updateGyroStats(const Vector3& gyro, float dt) {
            ...
            // Simple moving average for bias
            if (sample_count == 1) {
                gyro_bias = gyro;
            } else {
                float alpha = 1.0f / sample_count;
                gyro_bias = gyro_bias * (1.0f - alpha) + gyro * alpha;
            }
            
            // Update variance
            Vector3 delta = gyro - gyro_bias;
            gyro_bias_var[0] = gyro_bias_var[0] * 0.99f + delta.x * delta.x * 0.01f;
            gyro_bias_var[1] = gyro_bias_var[1] * 0.99f + delta.y * delta.y * 0.01f;
            gyro_bias_var[2] = gyro_bias_var[2] * 0.99f + delta.z * delta.z * 0.01f;
        }
    } _bias_estimator;
    
    // 5. Innovation monitor
    struct InnovationMonitor {
        ...
        static constexpr float CHI2_THRESHOLD_3D = 7.815f;  // 95% confidence
        ...
        bool checkInnovation(const Vector3& innovation, const Eigen::Matrix3f& S) {
            ...
            if (fabs(det) < 1e-6f) {
                accel_test_ratio = 100.0f;  // Large value for bad covariance
                accel_healthy = false;
                return false;
            }
            ...
            // For 3D measurement, chi-square 95% threshold is 7.815
            accel_test_ratio = nis / CHI2_THRESHOLD_3D;
            accel_healthy = (accel_test_ratio < 1.0f);
    ...
    struct CovarianceLimiter {
        // Change from static constexpr to regular array
        const float MAX_STATE_VAR[23] = {
            ...
            0.01f, 0.01f, 0.01f,   // Gyro bias (rad²/s²)
...
public:
    ...
    : delta_angle_bias_(0, 0, 0),
      ...
      config() {
        ...
        // Replace initialization of quat_nominal_ with:
        _states.quat_nominal = Quaternion(1, 0, 0, 0);
        _states.vel_NED = Vector3(0, 0, 0);
        _states.pos_NED = Vector3(0, 0, 0);
        _states.delta_angle_bias = Vector3(0, 0, 0);
        _states.delta_vel_bias = Vector3(0, 0, 0);

        // Initialize error covariance matrix
        // P = Eigen::Matrix<double, 6, 6>::Identity() * 0.01;

        // ADD: Resize P matrix to handle new states
        P = Eigen::Matrix<float, 23, 23>::Identity() * 1e-6f;  // CHANGE from 6x6 to 23x23
        ...
        // Initialize new PX4-style state variables
        error_state_.setZero();
        quat_nominal_ = Quaternion(1, 0, 0, 0);  // Identity quaternion
        vel_NED_ = Vector3(0, 0, 0);
        pos_NED_ = Vector3(0, 0, 0);
        delta_angle_bias_ = Vector3(0, 0, 0);
        delta_vel_bias_ = Vector3(0, 0, 0);
        mag_I_ = Vector3(0, 0, 1);  // Default earth field pointing up
        mag_B_ = Vector3(0, 0, 0);
        wind_vel_ = Vector2(0, 0);
        P.setIdentity();
        P *= 0.1f;  // Initial uncertainty
        Q.setZero();
    }
    ...
    void fuseAccelerometerRobust(const Vector3& accel_body, uint64_t time_us) {
        ...
        StateSnapshot* state_at_meas = nullptr;
        for (auto& snapshot : _state_history) {
            if (snapshot.time_us >= time_us) {
                state_at_meas = &snapshot;
                break;
            }
        }
        
        if (!state_at_meas) return;
        
        // Expected gravity in body frame
        Vector3 gravity_ned(0, 0, 9.81f);
        Eigen::Matrix3f R_ned_to_body = state_at_meas->quat.toMatrix3f().transpose();
        ...
        Eigen::Matrix3f S = H * state_at_meas->P * H.transpose() + R_accel;
        ...
        // Kalman gain
        Eigen::Matrix<float, 23, 3> K = state_at_meas->P * H.transpose() * S.inverse();
        
        // State update
        Eigen::Matrix<float, 23, 1> state_correction = K * innovation.toEigen();
        
        // Apply corrections
        applyDelayedCorrection(state_at_meas, state_correction);
        
        // Joseph form covariance update
        Eigen::Matrix<float, 23, 23> I_KH = 
            Eigen::Matrix<float, 23, 23>::Identity() - K * H;
        state_at_meas->P = I_KH * state_at_meas->P * I_KH.transpose() + 
                          K * R_accel * K.transpose();
        
        // Propagate corrections forward
        propagateCorrectionsForward(time_us);
    }
    ...
    void stabilizeYaw(double dt) {
        ...
        if (is_static && std::abs(yaw_velocity) > 0.01) {
            ...
            // Apply correction
            quat_nominal_ = yaw_correction * quat_nominal_;
            quat_nominal_.normalize();
        }
    }
    ...
    void updateGyroBias(const Vector3& gyro_raw, double dt) {
        ...
        if (is_static) {
            ...
            Vector3 gyro_error = gyro_raw - delta_angle_bias_;
            ...
            if (static_counter >= 50) {
                ...
                delta_angle_bias_ += avg_gyro * bias_lr;
    ...
    void update(const SensorData& data, float dt) {
        ...
        if (!initialized) {
            accel_buffer.push_back(Vector3(data.accel_x, data.accel_y, data.accel_z));
            gyro_buffer.push_back(Vector3(data.gyro_x, data.gyro_y, data.gyro_z));
        ...
        // IMPORTANT: Use delayed fusion to populate buffers
        updateWithDelayedFusion(data, dt);
        
        // Also do immediate predict-correct for comparison
        Vector3 gyro(data.gyro_x, data.gyro_y, data.gyro_z);
        Vector3 accel(data.accel_x, data.accel_y, data.accel_z);
    ...
    void detectStaticState(const Vector3& gyro, double dt) {
        ...
        bool currently_static = (gyro_mag < 0.01);  // 0.01 rad/s threshold
    ...
    void initializeFilter(const SensorData& data) {
        // Buffer some samples for better initialization
        static std::vector<Vector3> accel_samples;
        static std::vector<Vector3> gyro_samples;
        
        accel_samples.push_back(Vector3(data.accel_x, data.accel_y, data.accel_z));
        gyro_samples.push_back(Vector3(data.gyro_x, data.gyro_y, data.gyro_z));
        
        if (accel_samples.size() < 10) {
        ...
        for (size_t i = 0; i < accel_samples.size(); i++) {
            accel_avg += accel_samples[i];
            gyro_avg += gyro_samples[i];
        }
        accel_avg = accel_avg / accel_samples.size();
        gyro_avg = gyro_avg / gyro_samples.size();
        
        // Initialize gyro bias from static samples
        delta_angle_bias_ = gyro_avg;
        
        // Initialize attitude from gravity
        double roll = atan2(accel_avg.y, accel_avg.z);
        double pitch = atan2(-accel_avg.x, 
                            sqrt(accel_avg.y*accel_avg.y + accel_avg.z*accel_avg.z));
        
        quat_nominal_ = Quaternion::fromEuler(roll, pitch, 0.0);
        if (quat_nominal_.w < 0) {
            quat_nominal_.w = -quat_nominal_.w;
            quat_nominal_.x = -quat_nominal_.x;
            quat_nominal_.y = -quat_nominal_.y;
        ...
        P = Eigen::Matrix<float, 23, 23>::Identity();
        P.block<3,3>(STATE_QUAT_X, STATE_QUAT_X) *= 0.1f;      // Attitude uncertainty
        P.block<3,3>(STATE_VEL_X, STATE_VEL_X) *= 1.0f;        // Velocity uncertainty
        P.block<3,3>(STATE_POS_X, STATE_POS_X) *= 10.0f;       // Position uncertainty
        P.block<3,3>(STATE_GYRO_BIAS_X, STATE_GYRO_BIAS_X) *= 0.01f;
        P.block<3,3>(STATE_ACCEL_BIAS_X, STATE_ACCEL_BIAS_X) *= 0.1f;
        ...
        std::cout << "Filter initialized with Roll: " << roll * RAD_TO_DEG 
    ...
    FilteredOutput getFilteredOutput() const {
        ...
        // Smooth roll transitions at ±π
        static double prev_roll = 0;
        if (std::abs(euler.x - prev_roll) > 5.0) {  // Detect jump (5 radians ~ 286 degrees)
            if (euler.x > 0) 
                euler.x -= 2*PI;
            else 
                euler.x += 2*PI;
        ...
        output.roll = euler.x;    // radians
        output.pitch = euler.y;   // radians
        output.yaw = euler.z;     // radians
        
        // Get quaternion components
        output.quat_w = quat_nominal_.w;
        output.quat_x = quat_nominal_.x;
        output.quat_y = quat_nominal_.y;
        output.quat_z = quat_nominal_.z;
...
void AdvancedEKF::imuUpdate(const Vector3& gyro, const Vector3& accel, float dt, uint64_t time_us) {
    ...
    // IMPORTANT: Actually process the buffer
    processFusionHorizon();
}
...
void AdvancedEKF::correctWithAccelerometer(const Vector3& accel_meas) {
    ...
    Vector3 expected_accel = nedToBody(gravity_NED);
    
    // 2. Compute innovation (y - h(x))
    Vector3 innovation = accel_meas - expected_accel;
    
    // 3. Compute measurement Jacobian (H)
    Eigen::Matrix<float, 3, 23> H;
    H.setZero();
    
    // Gravity measurement is affected by attitude errors
    Eigen::Matrix3f R = quat_nominal_.toMatrix3f();
    Vector3f g_body = R.transpose() * Vector3f(0, 0, GRAVITY_MSS);
    H.block<3,3>(0, STATE_QUAT_X) = skewSymmetric(g_body);
    
    // Accelerometer bias affects measurement directly
    ...
    // 6. Update error state: δx = K * innovation
    error_state_ = K * innovation.toEigen();
    
    // 7. Update covariance (Joseph form for numerical stability)
    Eigen::Matrix<float, 23, 23> I_KH = Eigen::Matrix<float, 23, 23>::Identity() - K * H;
    P = I_KH * P * I_KH.transpose() + K * R_accel * K.transpose();
    P = 0.5f * (P + P.transpose());  // Ensure symmetry
    
    // 8. Apply error state corrections to nominal state
...
void AdvancedEKF::compensateLinearAcceleration(Vector3& accel) {
    ...
    if (linear_accel_mag > 3.0) {
        Vector3 predicted_gravity = quat_nominal_.rotate(Vector3(0, 0, 9.81));
