...
public:
    ...
    : delta_angle_bias_(0, 0, 0),
      ...
      config() {
        ...
        mag_I_ = Vector3(0, 0, -1);  // Default earth field pointing down
    ...
    void update(const SensorData& data, float dt) {
        ...
        // ADD THIS NEW PX4-STYLE PREDICTION AT THE BEGINNING:
        if (initialized) {
            // Create ImuSample from SensorData for new prediction
            ImuSample imu_sample;
            imu_sample.time_us = static_cast<uint64_t>(data.timestamp * 1e6);
            imu_sample.delta_angle = Vector3(data.gyro_x * dt, data.gyro_y * dt, data.gyro_z * dt);
            imu_sample.delta_velocity = Vector3(data.accel_x * dt, data.accel_y * dt, data.accel_z * dt);
            imu_sample.dt = dt;
            
            // Call new PX4-style prediction
        ...
        // Apply bias corrections
        Vector3 corrected_delta_angle = delta_angle - delta_angle_bias_ * dt;
        ...
        Vector3 delta_vel_NED = quat_nominal_.rotateToNED(corrected_delta_vel);
        ...
        if (accel_norm > 0.1 && std::abs(accel_norm - 9.81) < 2.0) {
            ...
            Vector3 expected_gravity_body = quat_nominal_.rotateToBody(gravity_NED);
            ...
            // Joseph update
            josephFormCovarianceUpdate<23, 3>(innovation, R_accel_meas, H);
