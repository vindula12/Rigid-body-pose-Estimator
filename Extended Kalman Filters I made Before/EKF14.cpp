...
public:
    ...
    struct InnovationMonitor {
        ...
        bool checkInnovation(const Vector3& innovation, const Eigen::Matrix3f& S) {
            ...
            // Calculate test ratio based on innovation magnitude
            accel_test_ratio = innovation_magnitude / 2.0f;  // Scale factor for testing
            
            // Determine health
            accel_healthy = (innovation_magnitude < 5.0f);
...
void AdvancedEKF::reset() {
    ...
    P = Eigen::Matrix<float, 23, 23>::Identity();
    P.block<3,3>(0, 0) *= 0.1f;      // Attitude
    P.block<3,3>(3, 3) *= 1.0f;      // Velocity  
    P.block<3,3>(6, 6) *= 10.0f;     // Position
    P.block<3,3>(9, 9) *= 0.005f;    // Gyro bias: UNDER 0.01 limit
    P.block<3,3>(12, 12) *= 0.05f;   // Accel bias
    P.block<6,6>(15, 15) *= 0.5f;    // Mag states
    P.block<2,2>(21, 21) *= 50.0f;   // Wind
...
void AdvancedEKF::update(const SensorData& data, float dt) {
    ...
    if (!initialized) {
        ...
        if (accel_buffer.size() >= 10) {
            ...
            P = Eigen::Matrix<float, 23, 23>::Identity();
            P.block<3,3>(0, 0) *= 0.1f;      // Attitude: 0.1
            P.block<3,3>(3, 3) *= 1.0f;      // Velocity: 1.0  
            P.block<3,3>(6, 6) *= 10.0f;     // Position: 10.0
            P.block<3,3>(9, 9) *= 0.005f;    // Gyro bias: 0.005 (under 0.01 limit)
            P.block<3,3>(12, 12) *= 0.05f;   // Accel bias: 0.05 (under 0.1 limit)
            P.block<6,6>(15, 15) *= 0.5f;    // Mag states: 0.5
            P.block<2,2>(21, 21) *= 50.0f;   // Wind: 50.0
