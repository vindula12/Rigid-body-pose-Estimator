...
public:
    ...
    void update(const SensorData& data, double dt) {
        ...
        if (!initialized) {
            if (accel_buffer.size() >= 20 && gyro_buffer.size() >= 20) {
...
void configureTheoreticalOptimal(AdvancedEKF& ekf) {
    ...
    config.accel_trust_base = 0.01;      // Balanced accelerometer trust
    config.gyro_trust_base = 0.2;        // Moderate gyro trust
    config.mag_trust_base = 0.002;       // Low magnetometer trust
    
    // Process noise tuned for stability
    config.process_noise_attitude = 1e-5;  // Very low for smooth output
    config.process_noise_bias = 1e-7;      // Even lower for bias stability
    ...
    config.smoothing_factor = 0.1;         // Moderate smoothing
...
void AdvancedEKF::improvedTheoreticalInitialization() {
    ...
    // Use more samples for robust initialization
    Vector3 gravity_sum(0, 0, 0);
    Vector3 gyro_bias_sum(0, 0, 0);
    
    // Calculate statistics over initialization period
    for (size_t i = 0; i < accel_buffer.size(); i++) {
        gravity_sum += accel_buffer[i];
        gyro_bias_sum += gyro_buffer[i];
    }
    
    Vector3 gravity_avg = gravity_sum / accel_buffer.size();
    gyro_bias = gyro_bias_sum / accel_buffer.size();
    ...
    // CRITICAL: Match PX4 coordinate system conventions
    // PX4 uses NED (North-East-Down) frame
    Vector3 gravity_normalized = gravity_avg.normalized();
    
    // Calculate roll and pitch using PX4 conventions
    ...
    // Initialize yaw based on magnetometer if available
    double yaw = 0.0;
    if (!mag_buffer.empty() && mag_buffer.size() >= 20) {
        Vector3 mag_sum(0, 0, 0);
        for (const auto& mag : mag_buffer) {
            mag_sum += mag;
        }
        Vector3 mag_avg = mag_sum / mag_buffer.size();
        
        if (mag_avg.norm() > 0.1) {
            // Transform to horizontal plane
            double cr = std::cos(roll);
            double sr = std::sin(roll);
            double cp = std::cos(pitch);
            double sp = std::sin(pitch);
    ...
    // Create initial quaternion using ZYX rotation order (matching PX4)
    q_state = Quaternion::fromEuler(roll, pitch, yaw);
    
    // Initialize with very low uncertainty for stable start
    P = Eigen::Matrix<double, 6, 6>::Identity() * 0.0001;
    P.block<3,3>(3,3) = Eigen::Matrix3d::Identity() * 0.00001; // Even lower for bias
    
    // Initialize all quaternion states
    smoothed_quaternion = q_state;
    prev_quaternion = q_state;
    prev_prev_quaternion = q_state;
    predicted_next_quaternion = q_state;
    
    // Set initial variance estimates
    accel_variance = Vector3(0.001, 0.001, 0.001);
    gyro_variance = Vector3(0.0001, 0.0001, 0.0001);
    
    initialized = true;
    orientation_stabilized = false;
    
    std::cout << "Improved initialization completed:" << std::endl;
    std::cout << "  Roll: " << roll * RAD_TO_DEG << " deg" << std::endl;
    std::cout << "  Pitch: " << pitch * RAD_TO_DEG << " deg" << std::endl;
    std::cout << "  Yaw: " << yaw * RAD_TO_DEG << " deg" << std::endl;
    std::cout << "  Initial quaternion: " << q_state.w << ", " << q_state.x 
              << ", " << q_state.y << ", " << q_state.z << std::endl;
}
...
void AdvancedEKF::advancedQuaternionIntegration(const Vector3& omega, double dt) {
    ...
    for (int step = 0; step < num_steps; step++) {
        ...
        Quaternion k1 = quat_derivative(q_state, omega);
    ...
    // Apply explicit re-normalization with high precision
    double q_norm = std::sqrt(q_state.w*q_state.w + q_state.x*q_state.x + 
                             q_state.y*q_state.y + q_state.z*q_state.z);
    
    if (q_norm > 1e-15) {
        double inv_norm = 1.0 / q_norm;
        q_state.w *= inv_norm;
        q_state.x *= inv_norm;
        q_state.y *= inv_norm;
        q_state.z *= inv_norm;
...
int main(int argc, char* argv[]) {
    ...
    for (size_t i = 0; i < sensor_data.size(); i++) {
        ...
        if (i > 0) {
            ...
            if (dt > 10.0) dt /= 1000.0; // Convert ms to seconds
            dt = std::max(0.001, std::min(0.1, dt)); // Clamp to reasonable range
        }
        
        // Use the theoretical optimal update method
