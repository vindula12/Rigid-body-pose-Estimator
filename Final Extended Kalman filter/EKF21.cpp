...
public:
    ...
    //     // IMPROVED: Using scipy.spatial.transform.Rotation equivalent
    //     // This matches the Python script's conversion method
        
    //     // Ensure quaternion is normalized
    //     double norm = std::sqrt(w*w + x*x + y*y + z*z);
    //     if (norm < 1e-12) {
    //         return Vector3(0, 0, 0);  // Identity case
    //     }
    ...
    //     // Convert to Euler angles using 'xyz' intrinsic rotation sequence
    //     // This matches scipy.spatial.transform.Rotation.as_euler('xyz')
        
    //     // Calculate rotation matrix elements we need
    ...
    //     double sin_pitch = r13;
    //     if (std::abs(sin_pitch) >= 1.0) {
    //         // Gimbal lock case
    //         pitch = std::copysign(PI/2, sin_pitch);
    //         yaw = std::atan2(-r21, r22);
    //         roll = 0.0;  // Set roll to zero in gimbal lock
    //     } else {
    //         // Normal case
    ...
    //     return Vector3(roll, pitch, yaw);  // Returns radians, same as Python
    // }

    // Exact same method used in python Scipy
    Vector3 toEulerAngles() const {
        // EXACT SCIPY ALGORITHM: from_quat().as_euler('xyz', degrees=False)
        ...
        // scipy internal conversion for 'xyz' Euler sequence
        // Reference: scipy/spatial/transform/_rotation.pyx
        
        double test = qx*qy + qz*qw;
        
        if (test > 0.4999999) {
            // Singularity at north pole
            double yaw = 2.0 * std::atan2(qx, qw);
            double pitch = PI / 2.0;
            double roll = 0.0;
        ...
        // Normal case - scipy's exact formulation
        double sqx = qx * qx;
        double sqy = qy * qy;
        double sqz = qz * qz;
        
        double yaw = std::atan2(2.0 * (qw*qz + qx*qy), 1.0 - 2.0 * (sqy + sqz));
        double pitch = std::asin(2.0 * test);
        double roll = std::atan2(2.0 * (qw*qx + qy*qz), 1.0 - 2.0 * (sqx + sqy));
        
        return Vector3(roll, pitch, yaw);  // Radians, same as scipy
    }

    Vector3 toEulerAnglesDegrees() const {
        Vector3 euler_rad = toEulerAngles();
        return Vector3(
            euler_rad.x * RAD_TO_DEG,
            euler_rad.y * RAD_TO_DEG,
            euler_rad.z * RAD_TO_DEG
        );
    }
...
public:
    ...
    FilteredOutput getFilteredOutput() const {
        ...
        // IMPROVED: Get Euler angles using scipy-equivalent method in RADIANS
        Vector3 euler = quat_nominal_.toEulerAngles();  // Returns radians
...
void writeOutputCSV(const std::string& filename, const std::vector<FilteredOutput>& output) {
    ...
    // Write header(Roll, Pitch, Yaw in radians)
