...
struct ImuSample {
    ...
    float delta_angle_dt;    // Make sure this field exists
    float delta_velocity_dt;  // Make sure this field exists
    float dt;                // This might be redundant, check your usage
};
...
public:
    ...
    void updateProcessNoise(float dt) {
        ...
        // Gyro bias random walk
        Q.block<3,3>(9, 9) = Eigen::Matrix3f::Identity() * (0.00001f * dt);
        
        // Accelerometer bias random walk
        Q.block<3,3>(12, 12) = Eigen::Matrix3f::Identity() * (0.0001f * dt);
    ...
    struct InnovationMonitor {
        ...
        // REPLACE the checkInnovation method in InnovationMonitor struct:
...
public:
    ...
    : delta_angle_bias_(0, 0, 0),
      ...
      accel_trust(0.02),                             // CHANGED: was (0.01)
      gyro_trust(0.98),                               // CHANGED: was (0.2)
      frame_count(0),
      initialized(false),
    ...
    void stabilizeYaw(double dt) {
        // Only stabilize during extended static periods
        if (!is_static || static_time < 1.0) return;
        
        Vector3 euler = quat_nominal_.toEulerAngles();
        
        // Very gentle yaw drift correction
        const double YAW_DRIFT_CORRECTION = 0.0001;
        double yaw_correction = -euler.z * YAW_DRIFT_CORRECTION * dt;
        
        // Limit correction
        yaw_correction = std::max(-0.001, std::min(0.001, yaw_correction));
        
        // Apply as rotation around Z axis
    ...
    void initializeFilter(const SensorData& data) {
        ...
        // CRITICAL FIX: Proper attitude initialization
        ...
        if (gravity_norm > 8.0 && gravity_norm < 12.0) {
            ...
            double pitch = std::atan2(-g_norm.x, 
                                    std::sqrt(g_norm.y * g_norm.y + g_norm.z * g_norm.z));
        ...
        // CRITICAL FIX: Initialize covariance with proper values
        P = Eigen::Matrix<float, 23, 23>::Identity();
        P.block<3,3>(STATE_QUAT_X, STATE_QUAT_X) *= 0.01f;
        P.block<3,3>(STATE_VEL_X, STATE_VEL_X) *= 1.0f;
        P.block<3,3>(STATE_POS_X, STATE_POS_X) *= 10.0f;
        P.block<3,3>(STATE_GYRO_BIAS_X, STATE_GYRO_BIAS_X) *= 0.001f;  // Critical
...
void testAdvancedEKF() {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        // Use the update method which calls imuUpdate internally
        ekf->update(data, 0.01);
    }

    // Now check buffer size
    std::cout << "IMU Buffer size: " << ekf->_imu_buffer.size() << " samples\n";
    std::cout << "State history size: " << ekf->_state_history.size() << " snapshots\n";
    std::cout << "Fusion delay: 100ms (configured)\n";
    std::cout << "Newest IMU time: " << ekf->_newest_imu_time_us << " us\n";
    ...
    // Check innovation monitor for normal data
    Vector3 normal_innovation(0.0, 0.0, 0.0);
    Eigen::Matrix3f S_normal = Eigen::Matrix3f::Identity() * 1.0f;
    ekf->_innov_monitor.checkInnovation(normal_innovation, S_normal);
    ...
    std::cout << "  Test ratio: " << std::fixed << std::setprecision(4) 
            << normal_ratio << " (should be < 1.0)\n";
    ...
    // Test the innovation monitor directly with large innovation
    Vector3 outlier_innovation(50.0, 0.0, -9.81);  // Large innovation
    Eigen::Matrix3f S_outlier = Eigen::Matrix3f::Identity() * 1.0f;
    ekf->_innov_monitor.checkInnovation(outlier_innovation, S_outlier);
...
void AdvancedEKF::update(const SensorData& data, float dt) {
    ...
    if (!initialized) {
        ...
        if (accel_buffer.size() >= 10) {
            initializeFilter(data);  // Keep your existing initialization
        }
        return;
    }
    ...
    // CRITICAL FIX: Replace lines 2094-2109 with proper quaternion update
    if (corrected_gyro.norm() > 1e-8) {
        float angle = corrected_gyro.norm() * dt;
        ...
        // CRITICAL: Add canonical form enforcement
        if (quat_nominal_.w < 0) {
            quat_nominal_ = -quat_nominal_;
        }
    }
    
    // CRITICAL FIX: Update velocity properly (replace lines 2111-2116)
    Eigen::Matrix3f R = quat_nominal_.toMatrix3f();
    Eigen::Vector3f corrected_accel_eigen(corrected_accel.x, corrected_accel.y, corrected_accel.z);
    Eigen::Vector3f accel_NED_eigen = R * corrected_accel_eigen;
    Vector3 accel_NED(accel_NED_eigen(0), accel_NED_eigen(1), accel_NED_eigen(2));
    
    vel_NED_ += accel_NED * dt;
    vel_NED_.z += GRAVITY_MSS * dt;  // Use the constant
    ...
    // CRITICAL FIX: Only apply accelerometer correction when not accelerating
    float accel_mag = accel_filtered.norm();
    if (accel_mag > 8.0f && accel_mag < 11.0f) {
        correctWithAccelerometer(accel_filtered);
    }
    
    // CRITICAL: Add yaw stabilization
    if (is_static && static_time > 1.0f) {
        stabilizeYaw(dt);
    }
