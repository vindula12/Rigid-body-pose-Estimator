...
public:
    ...
    struct InnovationMonitor {
        ...
        bool checkInnovation(const Vector3& innovation, const Eigen::Matrix3f& S) {
            ...
            if (fabs(det) < 1e-6f) {
                accel_test_ratio = 10.0f;  // Large value for bad covariance
                accel_healthy = false;
                return false;
            }
...
public:
    ...
    // Update method that processes sensor data
    // and applies the Extended Kalman Filter logic
    // void update(const SensorData& data, float dt) {
    //     // Use the new delayed fusion update
    ...
    void update(const SensorData& data, float dt) {
        ...
        if (initialized) {
            // Update bias estimator for tests
            Vector3 gyro(data.gyro_x, data.gyro_y, data.gyro_z);
            if (gyro.norm() < 0.01f) {
                _bias_estimator.updateGyroStats(gyro, dt);
                
                // Copy to main bias estimate after convergence
                if (static_time > 1.0f) {
                    delta_angle_bias_ = _bias_estimator.gyro_bias;
                }
            }
            
            // Update innovation monitor
            Vector3 accel(data.accel_x, data.accel_y, data.accel_z);
            if (fabs(accel.norm() - 9.81f) < 2.0f) {
                // Expected gravity
                Vector3 gravity_ned(0, 0, 9.81f);
                Vector3 expected_gravity = quat_nominal_.conjugate().rotate(gravity_ned);
                Vector3 innovation = accel - expected_gravity;
                
                // Create simple S matrix for test
                Eigen::Matrix3f S = Eigen::Matrix3f::Identity() * 0.5f;
                _innov_monitor.checkInnovation(innovation, S);
            }
        }
        
        // Call the delayed fusion update instead of direct update
        updateWithDelayedFusion(data, dt);
    }
    ...
    void initializeOrientation(const Vector3& accel) {
        ...
        // For NED frame with Z pointing down (positive gravity)
        double roll = std::atan2(accel_normalized.y, accel_normalized.z);
        double pitch = std::atan2(-accel_normalized.x, 
                                std::sqrt(accel_normalized.y * accel_normalized.y + accel_normalized.z * accel_normalized.z));
        ...
        // IMPORTANT: Ensure positive scalar part for canonical form
        if (quat_nominal_.w < 0) {
            quat_nominal_.w = -quat_nominal_.w;
            quat_nominal_.x = -quat_nominal_.x;
            quat_nominal_.y = -quat_nominal_.y;
        ...
        std::cout << "Initialized quaternion: [" << quat_nominal_.w << ", " 
                << quat_nominal_.x << ", " << quat_nominal_.y << ", " 
                << quat_nominal_.z << "]" << std::endl;
    }
    ...
    void correctWithAccelerometer(const Vector3& accel) {
        ...
        // Expected gravity in body frame (NED convention: gravity points down = positive Z)
        Vector3 gravity_ned(0, 0, 1);  // Unit gravity vector in NED
        Vector3 expected_gravity_body = quat_nominal_.conjugate().rotate(gravity_ned);
        
        // Calculate error (small angle approximation)
        Vector3 error = expected_gravity_body.cross(accel_normalized);
        
        // Apply correction with small gain
        float correction_gain = 0.01f;  // Small gain for stability
        
        if (error.norm() > 1e-8) {
            Quaternion correction = Quaternion::fromAxisAngle(error, error.norm() * correction_gain);
        ...
        //     delta_angle_bias_.x += error.x * 0.0003;
        //     delta_angle_bias_.y += error.y * 0.0003;
        //     delta_angle_bias_.z += error.z * 0.0003;
