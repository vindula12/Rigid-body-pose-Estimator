...
public:
    ...
    void update(const SensorData& data, float dt) {
        ...
        if (!initialized) {
            ...
            if (accel_buffer.size() >= 10) {
                ...
                // FIX: Use correct gravity convention
                ...
                _states.quat_nominal = Quaternion::fromEuler(roll, pitch, 0.0);
                
                // FIX: Ensure canonical form
                if (_states.quat_nominal.w < 0) {
                    _states.quat_nominal.w = -_states.quat_nominal.w;
                    _states.quat_nominal.x = -_states.quat_nominal.x;
                    _states.quat_nominal.y = -_states.quat_nominal.y;
                    _states.quat_nominal.z = -_states.quat_nominal.z;
                }
        ...
        // Convert to delta angles
        Vector3 delta_angle(data.gyro_x * dt, data.gyro_y * dt, data.gyro_z * dt);
        Vector3 delta_vel(data.accel_x * dt, data.accel_y * dt, data.accel_z * dt);
        
        // Apply bias corrections
        Vector3 corrected_delta_angle = delta_angle - _states.delta_angle_bias * dt;
        Vector3 corrected_delta_vel = delta_vel - _states.delta_vel_bias * dt;
        ...
        if (corrected_delta_angle.norm() > 1e-8) {
            ...
            // FIX: Use proper quaternion multiplication
            _states.quat_nominal = _states.quat_nominal * delta_q;
            _states.quat_nominal.normalize();
            
            // FIX: Enforce canonical form
            if (_states.quat_nominal.w < 0) {
                _states.quat_nominal.w = -_states.quat_nominal.w;
                _states.quat_nominal.x = -_states.quat_nominal.x;
                _states.quat_nominal.y = -_states.quat_nominal.y;
        ...
        Matrix3f R_body_to_ned = quatToRotationMatrix(_states.quat_nominal);
        Vector3 delta_vel_ned(
            R_body_to_ned(0,0)*corrected_delta_vel.x + R_body_to_ned(0,1)*corrected_delta_vel.y + R_body_to_ned(0,2)*corrected_delta_vel.z,
            R_body_to_ned(1,0)*corrected_delta_vel.x + R_body_to_ned(1,1)*corrected_delta_vel.y + R_body_to_ned(1,2)*corrected_delta_vel.z,
            R_body_to_ned(2,0)*corrected_delta_vel.x + R_body_to_ned(2,1)*corrected_delta_vel.y + R_body_to_ned(2,2)*corrected_delta_vel.z
        );
        
        _states.vel_NED += delta_vel_ned;
        _states.vel_NED.z += GRAVITY_MSS * dt;  // FIX: Positive gravity in NED
        
        // Update position
        _states.pos_NED += _states.vel_NED * dt;
        
        // Accelerometer fusion
        Vector3 accel_body(data.accel_x, data.accel_y, data.accel_z);
        float accel_norm = accel_body.norm();
        
        if (fabs(accel_norm - GRAVITY_MSS) < 2.0f) {
            // FIX: Correct gravity transformation
            Vector3 gravity_ned(0, 0, GRAVITY_MSS);  // Positive down
            Matrix3f R_ned_to_body = R_body_to_ned.transpose();
            Vector3 expected_gravity_body(
                R_ned_to_body(0,0)*gravity_ned.x + R_ned_to_body(0,1)*gravity_ned.y + R_ned_to_body(0,2)*gravity_ned.z,
                R_ned_to_body(1,0)*gravity_ned.x + R_ned_to_body(1,1)*gravity_ned.y + R_ned_to_body(1,2)*gravity_ned.z,
                R_ned_to_body(2,0)*gravity_ned.x + R_ned_to_body(2,1)*gravity_ned.y + R_ned_to_body(2,2)*gravity_ned.z
            );
            
            // Innovation
            Vector3 error = expected_gravity_body.cross(accel_body.normalized());
            
            // Apply correction
            float correction_gain = 0.01f;  // Small gain for stability
            if (error.norm() > 1e-6 && error.norm() < 0.5f) {
                Quaternion correction = Quaternion::fromAxisAngle(error, correction_gain);
                _states.quat_nominal = correction * _states.quat_nominal;
                _states.quat_nominal.normalize();
        ...
        // Update for output
        quat_nominal_ = _states.quat_nominal;  // For compatibility
        vel_NED_ = _states.vel_NED;
        pos_NED_ = _states.pos_NED;
        delta_angle_bias_ = _states.delta_angle_bias;
        delta_vel_bias_ = _states.delta_vel_bias;
    }
    ...
    FilteredOutput getFilteredOutput() const {
        ...
        // Get Euler angles
        Vector3 euler = _states.quat_nominal.toEulerAngles();
